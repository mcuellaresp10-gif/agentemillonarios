...
export function BuscadorGlobal({ compact }: { compact?: boolean }) {
  ...
  return (
    <form
      className={compact ? 'relative w-full' : 'flex gap-2 max-w-xl'}
      onSubmit={(e) => {
        e.preventDefault()
        search()
      }}
    >
      ...
      <Input
        ...
        className="pl-9 bg-white/10 border-white/20 text-white placeholder:text-blue-200 focus:bg-white focus:text-slate-900"
      ...
      {!compact && (
        <button
          type="submit"
          disabled={loading}
          className="rounded-md bg-mill-gold px-4 text-sm font-semibold text-mill-blue"
        >
          {loading ? '...' : 'Buscar'}
        </button>
      )}
